...
const QuizSearch = ({ quizzes, onPlay, onEdit, variant = "default", onExpandChange }: QuizSearchProps) => {
    ...
    return (
        <div ref={containerRef} className={`relative flex items-center transition-all duration-300 ease-in-out ${
            isNavbar 
            ...
            <div 
                className={`relative w-full flex items-center overflow-hidden transition-all duration-300 ${
                    isNavbar ? "rounded-full" : "rounded-2xl"
                } ${isNavbar && !isExpanded ? "cursor-pointer" : ""}`}
                onClick={handleToggleExpand}
            >
                <div className={`absolute left-0 w-10 h-10 flex items-center justify-center text-muted-foreground group-focus-within:text-primary transition-colors ${
                    isNavbar && !isExpanded ? "hover:bg-white/10 rounded-full" : "pl-1"
                }`}>
                ...
                <Input
                    ...
                    className={`transition-all duration-300 bg-card/60 backdrop-blur-xl border-2 border-white/10 shadow-xl focus-visible:ring-0 focus:border-primary font-bold placeholder:text-muted-foreground placeholder:font-normal ${
                        isNavbar 
            ...
            {isOpen && isExpanded && (query.trim() || results.length > 0) && (
                <div className={`absolute top-full mt-3 left-0 right-0 max-h-[450px] overflow-y-auto bg-card/95 backdrop-blur-2xl border-2 border-white/10 rounded-3xl shadow-2xl z-100 animate-in fade-in slide-in-from-top-2 duration-300 ${
                    isNavbar ? "md:w-[400px] w-[calc(100vw-2rem)]" : "w-full"
                }`}>
