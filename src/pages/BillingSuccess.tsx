...
export default function BillingSuccess() {
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center p-6">
      <div className={`max-w-md w-full bg-card/40 backdrop-blur-2xl border-2 ${error ? 'border-rose-500/20' : 'border-primary/20'} rounded-[3rem] p-10 text-center shadow-2xl space-y-8`}>
        <div className="flex justify-center">
          {error ? (
            <div className="w-24 h-24 rounded-full bg-rose-500/10 flex items-center justify-center text-rose-500">
              <AlertCircle size={60} strokeWidth={3} />
            </div>
          ) : (
        ...
        <div className="space-y-2">
          <h1 className="text-4xl font-black tracking-tighter text-foreground flex items-center justify-center gap-2">
            {error ? (
              ...
              <>Thanh toán <span className="text-emerald-500 italic">Thành công</span> <PartyPopper className="text-yellow-500" /></>
            )}
          </h1>
